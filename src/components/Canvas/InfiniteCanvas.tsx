...
interface InfiniteCanvasProps {
  ...
  onRequestPageCard?: (x: number, y: number) => void;
}
...
>(function InfiniteCanvas(
  ...
  // Initialize canvas pan/zoom — pass state (not ref) so hook re-runs when canvas is created
  const { zoomIn, zoomOut, resetView, fitToContent } = useCanvasPanZoom({
  ...
  useEffect(() => {
    ...
    if (initialContent?.fabricData) {
      ...
      canvas.loadFromJSON(initialContent.fabricData).then(() => {
        ...
        // Re-attach callbacks lost during deserialization
        rehydrateCustomObjects(canvas);
    ...
    // Throttled grid redraw on viewport change
    let lastGridDraw = 0;
    const throttledGridRedraw = () => {
      const now = Date.now();
      if (now - lastGridDraw > 100) {
        lastGridDraw = now;
        drawGrid();
      }
    };
    canvas.on("after:render", throttledGridRedraw);
    ...
    return () => {
      ...
      canvas.off("after:render", throttledGridRedraw);
      canvas.dispose();
  ...
  const updateConnections = useCallback(
    (movedObject: fabric.FabricObject | undefined) => {
      if (!movedObject || !fabricRef.current) return;

      const canvas = fabricRef.current;
      const connections = canvas
        .getObjects()
        .filter((obj) => obj instanceof ConnectionObject) as ConnectionObject[];
      ...
      connections.forEach((conn) => {
        if (conn.sourceId === objectId || conn.targetId === objectId) {
          const source = canvas
            ...
            .find(
              (obj) =>
                (obj as TextCardObject | PageCardObject).cardId ===
                conn.sourceId
            );
          const target = canvas
            .getObjects()
  ...
  // Re-attach callbacks and event handlers to custom objects after deserialization
  const rehydrateCustomObjects = useCallback(
    (canvas: fabric.Canvas) => {
      canvas.getObjects().forEach((obj) => {
        if (obj instanceof PageCardObject) {
          obj.setOnNavigate(onNavigateToPage ?? (() => {}));
        }
      });
    },
    [onNavigateToPage]
  ...
  useEffect(() => {
    ...
    // Reset connection state when switching away from connection tool
    if (selectedTool !== "connection") {
      ...
      // Restore default cursor (unless pan tool handles it)
      if (selectedTool !== "pan") {
  ...
  useEffect(() => {
    ...
    const handleSnap = (
      ...
      if (!settings.snapToGrid || !e.target) return;
      const gridSize = settings.gridSize;
      const obj = e.target;
      obj.set({
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (opt: fabric.TPointerEventInfo) => {
      ...
      if (selectedTool === "connection") {
        ...
        if (
          target &&
          (target instanceof TextCardObject || target instanceof PageCardObject)
        ) {
          if (!connectionSourceRef.current) {
            // First click - set source
            connectionSourceRef.current = target;
            target.set("stroke", "#60a5fa");
            setConnectionState("source-selected");
            canvas.renderAll();
          } else if (connectionSourceRef.current !== target) {
            // Second click - create connection
            const sourceId = (
              connectionSourceRef.current as TextCardObject | PageCardObject
            ).cardId;
  ...
  const addShapeAtPoint = useCallback(
    (tool: CanvasToolType, x: number, y: number) => {
      ...
      // Snap placement to grid if enabled
      ...
      switch (tool) {
        ...
        case "pageCard":
          // Delegate to parent to open page picker dialog
          onRequestPageCard?.(x, y);
          return;
      }
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // Tool switching shortcuts (single key, no modifiers)
      if (!e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
        const toolMap: Record<string, CanvasToolType> = {
          v: "select",
          h: "pan",
          p: "pen",
          e: "eraser",
  ...
  }, [undo, redo, deleteSelected, setConnectionState, setSelectedTool]);
