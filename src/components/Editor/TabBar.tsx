...
interface TabBarProps {
  onTabSelect: (pageId: string) => void;
}

export function TabBar({ onTabSelect }: TabBarProps) {
  const {
    openTabs,
    activeTabId,
    setActiveTab,
    ...
    <>
        ...
        >
          {openTabs.map((tab) => (
            <Tab
              key={tab.pageId}
              tab={tab}
              isActive={tab.pageId === activeTabId}
      ...
      {contextMenu && (
        <TabContextMenu
          ...
          isPinned={openTabs.find((t) => t.pageId === contextMenu.tabId)?.isPinned || false}
...
function Tab({
  ...
  return (
    <div
      className="tab-item group flex items-center gap-1 rounded-t-md px-3 py-1.5 cursor-pointer transition-colors"
      style={{
