    ...
    (set, get) => ({
      ...
      loadPages: async (notebookId, includeArchived) => {
        ...
        try {
          ...
          const pages = loadedPages.filter((p) => {
            if (seen.has(p.id)) {
              console.warn(
                "[pageStore] Duplicate page ID from API:",
                p.id,
                p.title
              );
              return false;
      ...
      createPage: async (
        ...
        try {
          ...
          set((state) => {
            ...
            const pages = [
              page,
              ...state.pages.filter((p) => p.id !== page.id),
            ];
      ...
      createSubpage: async (notebookId, parentPageId, title) => {
        ...
        try {
          ...
          set({
            error:
              err instanceof Error ? err.message : "Failed to create subpage",
          });
          return null;
        }
      },
      ...
      duplicatePage: async (notebookId, pageId) => {
        ...
        try {
          ...
          set((state) => {
            ...
            const pages = [
              newPage,
              ...state.pages.filter((p) => p.id !== newPage.id),
            ];
          ...
          useRAGStore
            .getState()
            .indexPage(notebookId, newPage.id)
            .catch(() => {});
        } catch (err) {
          set({
            error:
      ...
      movePageToParent: async (notebookId, pageId, parentPageId, position) => {
        ...
        try {
          ...
          set({
            error: err instanceof Error ? err.message : "Failed to move page",
          });
        }
      },
      ...
      archivePage: async (notebookId, pageId) => {
        ...
        try {
          ...
          set({
            error:
              err instanceof Error ? err.message : "Failed to archive page",
          });
        }
      },
      ...
      unarchivePage: async (notebookId, pageId, targetFolderId) => {
        set({ error: null });
        try {
          const page = await api.unarchivePage(
            notebookId,
            pageId,
            targetFolderId
          );
          ...
          set({
            error:
              err instanceof Error ? err.message : "Failed to unarchive page",
          });
        }
      },
        ...
        try {
          ...
          set({
            error:
              err instanceof Error
                ? err.message
                : "Failed to move page to section",
          });
        }
      },
        ...
        set((state) => ({
          panes: state.panes.map((pane) => {
            ...
            if (pageId && page && !pane.tabs.find((t) => t.pageId === pageId)) {
              newTabs = [
                ...pane.tabs,
                { pageId, title: page.title, isPinned: false },
              ];
            }
      ...
      closePane: (paneId) => {
        ...
        if (state.activePaneId === paneId) {
          const newActivePane =
            newPanes[Math.min(paneIndex, newPanes.length - 1)];
      ...
      splitPane: (paneId, _direction) => {
        ...
        const currentTab = sourcePane.tabs.find(
          (t) => t.pageId === sourcePane.pageId
        );
        const tabs: PaneTab[] = currentTab ? [{ ...currentTab }] : [];
        const newPane: EditorPane = {
          id: generatePaneId(),
          pageId: sourcePane.pageId,
          tabs,
        };
        ...
        set((state) => ({
          panes: state.panes.map((pane) => {
            ...
            const newTabs = pane.tabs.filter(
              (t) => t.pageId === keepPageId || t.isPinned
            );
      ...
      closeAllTabsInPane: (paneId) => {
        set((state) => ({
          panes: state.panes.map((pane) => {
            ...
            const newPageId =
              pinnedTabs.length > 0 ? pinnedTabs[0].pageId : null;
      ...
      selectTabInPane: (paneId, pageId) => {
        ...
        set((state) => ({
          ...
          selectedPageId:
            state.activePaneId === paneId ? pageId : state.selectedPageId,
        }));
      ...
      toggleFavorite: async (notebookId, pageId) => {
        ...
        try {
          const updatedPage = await api.updatePage(notebookId, pageId, {
