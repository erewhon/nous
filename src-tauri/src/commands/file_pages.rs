...
pub fn get_file_content(
    ...
    // Only allow reading text-based files (Jupyter, Chat, and Canvas are JSON, so also text-based)
    match page.page_type {
        PageType::Markdown | PageType::Calendar | PageType::Jupyter | PageType::Chat | PageType::Canvas => {}
        _ => {
            return Err(CommandError {
                message: format!(
...
pub fn update_file_content(
    ...
    // Only allow writing text-based files (Jupyter, Chat, and Canvas are JSON, also writable)
    match page.page_type {
        PageType::Markdown | PageType::Calendar | PageType::Jupyter | PageType::Chat | PageType::Canvas => {}
        _ => {
            return Err(CommandError {
