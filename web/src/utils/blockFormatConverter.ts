 ...
 *
 * Copied from src/utils/blockFormatConverter.ts with inline type definitions
 * to avoid the Zod dependency in the web bundle.
 */

// ─── Inline types (replaces Zod-based imports from src/types/page.ts) ───────
        ...
        blocks: [
          {
            ...
            type: "plugin",
            data: {
              pluginId: block.props?.pluginId ?? "",
              blockType: block.props?.blockType ?? "",
              dataJson: block.props?.dataJson ?? "{}",
            },
          },
        ],
        nextIndex: index + 1,
