/**
 * ChecklistTool for the guest editor.
 * Adapted from src/components/Editor/ChecklistTool.ts with Tauri dependencies removed.
 */
import type {
  BlockTool,
...
export class ChecklistTool implements BlockTool {
  ...
  private toggleItem(itemEl: HTMLElement, index: number): void {
    ...
    // In the guest editor, dispatch a custom event so GuestApp can trigger
    // an onChange-like save to propagate the check state via Yjs.
